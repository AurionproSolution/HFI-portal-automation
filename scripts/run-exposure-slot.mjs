...
if (!ids) {
  console.error("Usage: node scripts/run-exposure-slot.mjs <1-5>");
  process.exit(1);
}
...
const env = {
  ...
  USERNAME: process.env.USERNAME || "PB00018",
  PASSWORD: process.env.PASSWORD || "Secure@Pass4",
};

console.log(`Running Exposure Settings Slot ${slot}: ${ids.join(", ")}`);
