#!/usr/bin/env python3
"""Generate DealerFinanceDashboardRegression.test.ts from catalog + hand-crafted implementations."""

from pathlib import Path

OUT = Path("tests/hfi-portal/Regression/Sprint1/DealerFinanceDashboardRegression.test.ts")
    ...
    test(`TC-035 Overdue POs count and value are called out separately @ui @high @us-dlr-007 @dealer-finance-dashboard @regression`, async ({ page }) => {
      try {
        ...
        await regressionStep("Verify overdue callout in PO Bucket section", async () => {
          ...
          const bucketText = await dash.poBucketSection.innerText().catch(() => text);
